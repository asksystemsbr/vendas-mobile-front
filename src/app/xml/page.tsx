...
export default function OrdersWithFilesScreen() {
  ...
  return (
    <div className="container mx-auto p-8 flex flex-col min-h-screen">
      <h1 className="text-lg font-bold mb-6">Pedidos Aprovados com XML</h1>
  
      <div className="grid gap-3">
        {pedidos.map((pedido) => {
          // Garante que pedido.id é válido antes de acessar arquivos[pedido.id]
      ...
      <button onClick={() => router.push("../dashboard")} className="bg-gray-500 text-white px-4 py-2 rounded mt-6">
        Voltar
      </button>
    </div>
  );
